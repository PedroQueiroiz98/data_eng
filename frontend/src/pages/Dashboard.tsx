      ...
      <PageHeader
        ...
        actions={
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md border border-surface-border bg-surface px-3 py-1.5 text-sm text-fg-muted hover:bg-surface-variant"
          >
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <section className="lg:col-span-2">
          <div className="mb-2 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-fg">Últimas execuções</h2>
          ...
          <Card padded={false}>
            {recent.length === 0 ? (
              <p className="px-4 py-6 text-sm text-fg-faint">Nenhuma execução recente.</p>
            ) : (
                ...
                {recent.map((e) => (
                  <li key={e.id}>
                    ...
                    >
                      ...
                      <span className="font-mono text-xs text-fg-muted">{e.id.slice(0, 8)}</span>
        ...
        <section>
          <h2 className="mb-2 text-sm font-semibold text-fg">Próximos agendamentos</h2>
          <Card padded={false}>
            {upcoming.length === 0 ? (
              <p className="px-4 py-6 text-sm text-fg-faint">Nenhum agendamento ativo.</p>
            ) : (
              <ul className="divide-y divide-surface-border">
                {upcoming.map((s) => (
                  <li key={s.id} className="flex items-center gap-2 px-4 py-2.5 text-sm">
                    <ScheduleIcon className="h-4 w-4 text-fg-faint" />
                    <span className="truncate text-fg">{wfName(s.workflow_id)}</span>
                    <span className="ml-auto whitespace-nowrap text-xs text-fg-faint">
          ...
          <h2 className="mb-2 mt-6 text-sm font-semibold text-fg">Serviços</h2>
          <Card padded={false}>
