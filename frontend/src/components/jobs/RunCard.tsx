...
export function RunCard({ job, pipelineName, runNumber }: Props) {
  const hasCounts = job.task_total > 0;
  return (
    <Link
      to={`/jobs/${job.id}`}
      className="surface block px-4 py-3 transition hover:border-primary/40 hover:shadow-e2"
    >
      <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="tabular-nums text-xs text-fg-muted">{fmtDuration(job.duration_ms)}</span>
        </div>
      </div>
    </Link>
  );
